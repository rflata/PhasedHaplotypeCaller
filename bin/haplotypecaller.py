#HaployperCaller
#Author: Ryan Lata
#Primary computational class of the alorgithm, compares the variants pulled from phased VCF to the predefined haplotypes in the locationData file
...
class haplotypecaller:
    ...
    #Logic:
        ...
        # 2. If intersection length for the definition is greater than current haplotype call, clear the haplotype call, and set it to new value
        # 3. If the percent match for definition is greater than current haplotype call, and the intersection length is equal or greater than the current haplotype call
            #clear, and set it the current value
        # 4. If the percent match, and intersect are equal there is not consensus in the call, append current value to dictionary do not clear the stored value
            ...
            if len(hap1) > 0: #Conditional for if there are no variants in the regions
                for line in locationData:
                    if '#' not in line:
                        ...
                        #Attempted change in percentmatch calculation to adjust for the length of the haplotype in the locationData file
                        #May not be the best way to do this, but haven't seen any issues yet
